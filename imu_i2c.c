...
_SINT16 InBuffer[3] = {0}; 
static _SINT32 OutBuffer[3] = {0};
static MPU9250_AvgTypeDef MPU9250_Filter[3];
 float MotionVal[9];   
uint8 stI2CMgr;
...
uint8 I2C_READ(_UINT8 SaveAdress,_UINT8 *rdData, _UINT8 Data_length) // sends bytes to slave
{
    if(stI2CMgr==IDLE)
    {
      I2C_1_MasterClearStatus(); /* Clear any previous status */
      I2C_1_MasterReadBuf(SaveAdress, rdData, Data_length, I2C_1_MODE_COMPLETE_XFER);
...
{
    static _UINT8 st_IMU;
	int32_t	TempGx = 0, TempGy = 0, TempGz = 0;
	uint8_t i;
    _SINT32 Sum = 0;
    float Temp = 0.0f;
    
     static _UINT8 calc_count;
	...
	switch(st_IMU)
	{
		case IMU_INIT:	
					   if(I2C_WRITE(GYRO_ADDRESS, &Gyro_Calib_init[0][0], 10))  // send all init data
								...
								break;
	    case IMU_CALIB_READ: if(I2C_READ(GYRO_ADDRESS, &Gyro_Read_Data[0],6))  // read all gyro values in Gyro_Calib_data 
						   {
	    ...
	    case IMU_PRES: 
            ...
            if(I2C_READ(GYRO_ADDRESS, &Pres_Read_data[0],22))  // CAL_AC1 to MD see below
        	{
        	  st_IMU=CALIB_PRES;
        	}
        	break; 
	...
	if(stI2CMgr==IDLE)	
    {
  	...
  	AC1 = ((int16_t)Pres_Read_data[0] <<8 | ((int16_t)Pres_Read_data[1]));

  	AC2 = ((int16_t)Pres_Read_data[2] <<8 | ((int16_t)Pres_Read_data[3]));
  	
  	AC3 = ((int16_t)Pres_Read_data[4] <<8 | ((int16_t)Pres_Read_data[5]));
  	
  	AC4 = ((uint16_t)Pres_Read_data[6] <<8 | ((uint16_t)Pres_Read_data[7]));
 
  	AC5 = ((uint16_t)Pres_Read_data[8] <<8 | ((uint16_t)Pres_Read_data[9]));
  
  	AC6 = ((uint16_t)Pres_Read_data[10] <<8 | ((uint16_t)Pres_Read_data[11])); 

  	B1 = ((int16_t)Pres_Read_data[12] <<8 | ((int16_t)Pres_Read_data[13])); 

  	B2 = ((int16_t)Pres_Read_data[14] <<8 | ((int16_t)Pres_Read_data[15])); 
  	
  	MB = ((int16_t)Pres_Read_data[16] <<8 | ((int16_t)Pres_Read_data[17]));
 
  	MC = ((int16_t)Pres_Read_data[18] <<8 | ((int16_t)Pres_Read_data[19]));

  	MD = ((int16_t)Pres_Read_data[20] <<8 | ((int16_t)Pres_Read_data[21]));
	...
	// altitube measurment sensor
			   case SEL_TEMP: 
				...
				case READ_TEMP: 
					...
					if(I2C_READ(BMP180_ADDR, &Alt_Read_Data[0][0],2))
					{
					   // read pressure
					   st_IMU=CALC_TEMP;
					}
				...
				case CALC_TEMP:
				       if(stI2CMgr==IDLE)	               
					   {
					     UT = ((int32_t)Alt_Read_Data[0][0] << 8) + (int32_t)Alt_Read_Data[0][1]; 
					     st_IMU=SEL_PRES;
					   }
					   break;
				case SEL_PRES:
				...
				case READ_PRES:
					...
					if(I2C_READ(BMP180_ADDR, &Alt_Read_Data[1][0],3))  // BMP180_ReadReg(CONTROL_OUTPUT, 3, &RegBuff[0]); 
					{
					   // read pressure
					   st_IMU=CALC_PRES;
					}
				...
				case CALC_PRES : 
					...
					{
					   UP = (((int32_t)Alt_Read_Data[0] << 16) + ((int32_t)Alt_Read_Data[1] << 8) + ((int32_t)Alt_Read_Data[2])) >> (8 -_oss); // uncompensated pressure value
...
void Read_From_ALL(void)
{
    ...
    static _UINT8 st_Read;
    static int16_t InBuffer[3] = {0}; 
	 static int32_t OutBuffer[3] = {0};
    _UBYTE i;
			switch(st_Read)
			{
				case READ_GYRO: 
					if(I2C_READ(GYRO_ADDRESS, &Gyro_Read_Data[0],6))  // read all gyro values in Gyro_Read_Data 
					{
							st_Read=CALC_GYRO;
					}
					break;
				case CALC_GYRO: 
					if(stI2CMgr==IDLE)	             
					 ...
					 break;
				case READ_ACC: 
					if(I2C_READ(ACCEL_ADDRESS, &Acc_Read_Data[0],6))  // read all gyro values in Acc_Read_Data 
					{
							st_Read=CALC_ACC;
					}
					break;
				...
				case BYPASS_GYRO:   //  
					  ...
					  if(I2C_WRITE(GYRO_ADDRESS, &Gyro_Calib_init[7][0],2))   //  GYRO_ADDRESS,0x37,0x02
					  {
							st_Read=START_MAG;  // swith to next case after 10 ms
					  }
					 break;
				case START_MAG:
					  if(I2C_WRITE(MAG_ADDRESS, Mag_Calib_Data[0],2))   //  GYRO_ADDRESS,0x37,0x02
					  {
							st_Read=READ_MAG; 
					  }
					 break; //MAG_ADDRESS,0x0A,0x01
				case READ_MAG: 
					if(I2C_READ(MAG_ADDRESS, &Mag_Read_Data[0], 6))  // read all gyro values in Mag_Read_Data 
					{
							st_Read=CALC_MAG;
					}
					break;
