...
// pressure sensor fglobles
 static _UINT8 Alt_Read_Data[2][3]; // stores temperature data [0][] and pressure data [1][]
  ...
  signed short int AC1, AC2, AC3, B1, B2, MB, MC, MD, _oss;  
 _UINT16 AC4, AC5, AC6;
 _SINT32 B5, UT, UP;
...
void MPU9250_Init(void) // should be called when os is running 50 ms task
{
   ...
   // static _UINT8 i2c_err_count; // stores the count of i2c tx failure
    float Temp = 0.0f;
	...
	switch(st_IMU)
	{
		case IMU_INIT:	// below case configures gyro and accelrometer
                        ...
                        // TODO: check if magneto direct can be included with this
						break;
	    case IMU_CALIB_READ: if(I2C_READ(GYRO_ADDRESS,Gyro_Temp_Acc_Reg[0],&Gyro_Temp_Acc_Read_Data[0],6))  // read all gyro values in Gyro_Calib_data 
						   { 
    ...
    _UBYTE i;
			switch(st_Read)
			{
				...
				case BYPASS_GYRO:   //  
					  ...
					  if(I2C_WRITE(GYRO_ADDRESS, &Gyro_Calib_Data[5][0],2))   //  GYRO_ADDRESS,0x37,0x02
					  {
				...
				case READ_MAG: 
					if(I2C_READ(MAG_ADDRESS,Mag_Reg[0], &Mag_Read_Data[0], 6))  // read all gyro values in Mag_Read_Data 
					{
					/*	if(reg_count>=5)
                         {
                            st_Read=CALC_MAG;
