/*******************************************************************************
* File Name: CLK_Co_Op_Timer_Clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_Start
********************************************************************************
*
* Summary:
...
void CLK_Co_Op_Timer_Clock_Start(void) 
{
    ...
    CLK_Co_Op_Timer_Clock_CLKEN |= CLK_Co_Op_Timer_Clock_CLKEN_MASK;
	CLK_Co_Op_Timer_Clock_CLKSTBY |= CLK_Co_Op_Timer_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_Stop
********************************************************************************
*
* Summary:
...
void CLK_Co_Op_Timer_Clock_Stop(void) 
{
    ...
    CLK_Co_Op_Timer_Clock_CLKEN &= (uint8)(~CLK_Co_Op_Timer_Clock_CLKEN_MASK);
	CLK_Co_Op_Timer_Clock_CLKSTBY &= (uint8)(~CLK_Co_Op_Timer_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLK_Co_Op_Timer_Clock_CLKEN & CLK_Co_Op_Timer_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLK_Co_Op_Timer_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_Co_Op_Timer_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_Co_Op_Timer_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_Co_Op_Timer_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_Co_Op_Timer_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLK_Co_Op_Timer_Clock_GetSourceRegister();
    uint16 oldDivider = CLK_Co_Op_Timer_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_Co_Op_Timer_Clock_CLKEN & CLK_Co_Op_Timer_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CLK_Co_Op_Timer_Clock_DIV_PTR, clkDivider);
                CLK_Co_Op_Timer_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CLK_Co_Op_Timer_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CLK_Co_Op_Timer_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_Co_Op_Timer_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CLK_Co_Op_Timer_Clock_DIV_PTR, clkDivider);
				CLK_Co_Op_Timer_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLK_Co_Op_Timer_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_Co_Op_Timer_Clock_MOD_SRC |= modeBitMask & (uint8)CLK_Co_Op_Timer_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_Co_Op_Timer_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_Co_Op_Timer_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_Co_Op_Timer_Clock_MOD_SRC & (uint8)(CLK_Co_Op_Timer_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLK_Co_Op_Timer_Clock_GetDividerRegister();
    uint8 oldSrc = CLK_Co_Op_Timer_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_Co_Op_Timer_Clock_MOD_SRC |= CYCLK_SSS;
        CLK_Co_Op_Timer_Clock_MOD_SRC =
            (CLK_Co_Op_Timer_Clock_MOD_SRC & (uint8)(~CLK_Co_Op_Timer_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_Co_Op_Timer_Clock_MOD_SRC & CLK_Co_Op_Timer_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLK_Co_Op_Timer_Clock_PHASE = clkPhase & CLK_Co_Op_Timer_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_Co_Op_Timer_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLK_Co_Op_Timer_Clock_PHASE & CLK_Co_Op_Timer_Clock_PHASE_MASK;
}
