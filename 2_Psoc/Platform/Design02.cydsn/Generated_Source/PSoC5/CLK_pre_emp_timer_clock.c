/*******************************************************************************
* File Name: CLK_pre_emp_timer_clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_Start
********************************************************************************
*
* Summary:
...
void CLK_pre_emp_timer_clock_Start(void) 
{
    ...
    CLK_pre_emp_timer_clock_CLKEN |= CLK_pre_emp_timer_clock_CLKEN_MASK;
	CLK_pre_emp_timer_clock_CLKSTBY |= CLK_pre_emp_timer_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_Stop
********************************************************************************
*
* Summary:
...
void CLK_pre_emp_timer_clock_Stop(void) 
{
    ...
    CLK_pre_emp_timer_clock_CLKEN &= (uint8)(~CLK_pre_emp_timer_clock_CLKEN_MASK);
	CLK_pre_emp_timer_clock_CLKSTBY &= (uint8)(~CLK_pre_emp_timer_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLK_pre_emp_timer_clock_CLKEN & CLK_pre_emp_timer_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLK_pre_emp_timer_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_pre_emp_timer_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_pre_emp_timer_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_pre_emp_timer_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_pre_emp_timer_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLK_pre_emp_timer_clock_GetSourceRegister();
    uint16 oldDivider = CLK_pre_emp_timer_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_pre_emp_timer_clock_CLKEN & CLK_pre_emp_timer_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CLK_pre_emp_timer_clock_DIV_PTR, clkDivider);
                CLK_pre_emp_timer_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CLK_pre_emp_timer_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CLK_pre_emp_timer_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_pre_emp_timer_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CLK_pre_emp_timer_clock_DIV_PTR, clkDivider);
				CLK_pre_emp_timer_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLK_pre_emp_timer_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_pre_emp_timer_clock_MOD_SRC |= modeBitMask & (uint8)CLK_pre_emp_timer_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_pre_emp_timer_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_pre_emp_timer_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_pre_emp_timer_clock_MOD_SRC & (uint8)(CLK_pre_emp_timer_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLK_pre_emp_timer_clock_GetDividerRegister();
    uint8 oldSrc = CLK_pre_emp_timer_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_pre_emp_timer_clock_MOD_SRC |= CYCLK_SSS;
        CLK_pre_emp_timer_clock_MOD_SRC =
            (CLK_pre_emp_timer_clock_MOD_SRC & (uint8)(~CLK_pre_emp_timer_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_pre_emp_timer_clock_MOD_SRC & CLK_pre_emp_timer_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLK_pre_emp_timer_clock_PHASE = clkPhase & CLK_pre_emp_timer_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_pre_emp_timer_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLK_pre_emp_timer_clock_PHASE & CLK_pre_emp_timer_clock_PHASE_MASK;
}
