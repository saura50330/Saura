...
void LCD1_WriteData(uint8 dByte) 
{
    ...
    LCD1_WrDatNib(nibble);
     ...
     CyDelayUs(LCD1_NIB_DELAY_US);
	 
    /* Write low nibble */
	nibble = dByte & LCD1_NIBBLE_MASK;
    LCD1_WrDatNib(nibble);
	
	CyDelayUs(LCD1_DATA_DELAY_US);
...
static void LCD1_WrCntrlNib(uint8 nibble) 
{
    ...
    *******************************************************************************/
    void LCD1_PrintNumber(uint16 value) 
    {
        ...
        char8 number[LCD1_NUMBER_OF_REMAINDERS];
        char8 temp[LCD1_NUMBER_OF_REMAINDERS];
        ...
        {
            temp[digIndex] = (value % LCD1_TEN) + '0';
            value /= LCD1_TEN;
            digIndex++;
        }
